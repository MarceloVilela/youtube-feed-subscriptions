...
const browserOptions = { args: ["--no-sandbox"], headless: false };
let width = 1200;
let height = 1000;
let iterationNum = 10;
...
app.get("/page/screenshot", async (request: Request, response: Response) => {
  try {
    console.log(chalk.bgGreen("route[begin] /page/screenshot"));

    let data = [];

    const { url, y } = request.query;
    const yCommaSplitted = Array.from(String(y).split(","));
    const yNumericPositions = yCommaSplitted.map((item) => Number(item));

    data = await screenshot(String(url), yNumericPositions);

    console.log(chalk.bgGreen("route[success] /page/screenshot"));
    return response.json(data);
  } catch (error) {
    console.log(chalk.bgRed("route[error] /page/screenshot"));
    console.log(error);
    return response.status(500).json({ error: JSON.stringify(error, null, 2) });
  }
});
